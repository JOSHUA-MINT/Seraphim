...
async function runTests() {
  out.innerHTML = '';
  log('═══════════════════════════════════════════', 'dim');
  log('  OPHANIM CRYPTO TEST SUITE', 'info');
  log('═══════════════════════════════════════════', 'dim');
  log('');
  ...
  try {
    ...
    const keys = await OphanimCrypto.generateKeyPair(
      'Alice', 'alice@ophanim.test', 'test-passphrase-123'
    );
    ...
    const fp = await OphanimCrypto.getFingerprint(publicKey);
  ...
  try {
    ...
    const plaintext = 'Hello from Ophanim! 🔐 This is a secret message.';
    const t0 = performance.now();
    encrypted = await OphanimCrypto.encryptMessage(plaintext, publicKey);
    const elapsed = (performance.now() - t0).toFixed(0);
  ...
  try {
    ...
    const t0 = performance.now();
    const decrypted = await OphanimCrypto.decryptMessage(
    ...
    if (decrypted === 'Hello from Ophanim! 🔐 This is a secret message.') {
  ...
  try {
    ...
    const t0 = performance.now();
    signed = await OphanimCrypto.signMessage(
  ...
  try {
    ...
    const t0 = performance.now();
    const result = await OphanimCrypto.verifySignature(signed, publicKey);
    const elapsed = (performance.now() - t0).toFixed(0);
    if (result.verified) {
  ...
  try {
    ...
    await OphanimCrypto.decryptMessage(encrypted, privateKey, 'wrong-password');
  ...
  try {
    ...
    const codes = new Set();
    for (let i = 0; i < 10; i++) {
      codes.add(OphanimCrypto.generateRoomCode());
    }
